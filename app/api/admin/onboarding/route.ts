...
import { onboardingChecklist } from '@/lib/onboarding';
...
export async function POST(request: NextRequest) {
  ...
  if (body.action === 'create') {
    ...
    const { data: employee } = await supabase.from('time_employees')
      .select('id,primary_location_id,active').eq('id', parsed.data.employeeId).maybeSingle();
    if (!employee || !employee.active || !canAccessLocation(session, employee.primary_location_id)) {
      return NextResponse.json({ message: 'You do not have access to that employee.' }, { status: 403 });
    }
    ...
    if (error || !record) {
      return NextResponse.json({ message: error?.code === '23505' ? 'This employee already has an onboarding checklist.' : error?.message || 'Unable to create onboarding.' }, { status: 400 });
    }
  ...
  if (body.action === 'toggle') {
    ...
    const { error } = await supabase.from('hr_onboarding_items').update({
      ...
      completed_by_name: parsed.data.completed ? session.name : null,
...
async function load(supabase: NonNullable<ReturnType<typeof getAdminClient>>, session: TimeUserSession) {
  ...
  const [{ data: employees, error: employeeError }, { data: records, error: recordError }] = await Promise.all([employeeQuery, recordQuery]);
  const error = employeeError || recordError;
  if (error) return NextResponse.json({ message: error.message }, { status: 500 });
  const sorted = (records || []).map((record: any) => ({
    ...record,
    hr_onboarding_items: [...(record.hr_onboarding_items || [])].sort((a, b) => a.sort_order - b.sort_order),
  }));
  return NextResponse.json({ employees: employees || [], records: sorted });
}
