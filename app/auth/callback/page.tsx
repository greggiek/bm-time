...
export default function AuthCallbackPage() {
  ...
  useEffect(() => {
    async function finishLogin() {
      ...
      if (error || !data.session?.access_token) return setMessage('Google login could not be completed. Return to login and try again.');
      ...
      if (!response.ok) { await supabase.auth.signOut(); return setMessage(result.message || 'This Google account is not authorized for BM OS.'); }
      router.replace(result.redirectTo || '/manager');
      router.refresh();
    }
    finishLogin().catch(() => setMessage('Google login could not be completed.'));
  }, [router]);
