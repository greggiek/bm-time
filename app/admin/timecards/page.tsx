...
type Summary = { employeeId:string; employeeNumber:string; employeeName:string; location:string; totalHours:number; breakHours:number; incomplete:boolean };
type EmployeeOption = { id:string; employeeNumber:string; name:string };
type PaidTimeOff = { id:string; employeeId:string; employeeNumber:string; employeeName:string; location:string; entryType:'vacation'|'sick'; entryDate:string; hours:number; note:string };
...
export default function TimecardsPage(){
  ...
  const [paidTimeOff,setPaidTimeOff]=useState<PaidTimeOff[]>([]);
  ...
  const filteredPaidTimeOff=useMemo(()=>paidTimeOff.filter(row=>!searchTerm||row.employeeName.toLowerCase().includes(searchTerm)||row.employeeNumber.toLowerCase().includes(searchTerm)),[paidTimeOff,searchTerm]);
  ...
  async function request(body:Record<string,unknown>={}){
    ...
    setPunches(data.punches||[]);setSummaries(data.summaries||[]);setEmployees(data.employees||[]);setPaidTimeOff(data.paidTimeOff||[]);setUser(data.user||user);setLoaded(true);
  }
  ...
  async function addPaidTimeOff(event:FormEvent<HTMLFormElement>){event.preventDefault();const form=new FormData(event.currentTarget);setLoading(true);setError('');setMessage('');try{await request({action:'create_paid_time_off',employeeId:form.get('employeeId'),entryType:form.get('entryType'),entryDate:form.get('entryDate'),hours:form.get('hours'),note:form.get('note')});event.currentTarget.reset();setMessage('Paid time off added to the spreadsheet.')}catch(err){setError(err instanceof Error?err.message:'Unable to add paid time off.')}finally{setLoading(false)}}
  ...
  function exportCsv(){
    const lines:any[][]=[['Timecard Summary'],['From',startDate],['Through',endDate],[],['Employee Number','Employee','Location','Paid Hours','Unpaid Break Hours','Incomplete Punch'],...filteredSummaries.map(r=>[r.employeeNumber,r.employeeName,r.location,r.totalHours.toFixed(2),r.breakHours.toFixed(2),r.incomplete?'Yes':'No']),[],['Paid Time Off'],['Employee Number','Employee','Location','Type','Date','Paid Hours','Note'],...filteredPaidTimeOff.map(r=>[r.employeeNumber,r.employeeName,r.location,r.entryType==='vacation'?'Vacation':'Sick',r.entryDate,r.hours.toFixed(2),r.note]),[],['Punch Detail'],['Employee Number','Employee','Location','Action','Date','Time'],...filteredPunches.map(p=>{const d=new Date(p.occurredAt);return[p.employeeNumber,p.employeeName,p.location,p.action==='clock_in'?'Clock In':'Clock Out',d.toLocaleDateString(),d.toLocaleTimeString([],{hour:'numeric',minute:'2-digit'})]})];
  ...
  return <ManagerShell brand="BM TIME" title="Weekly Timecards" user={user}>
    <section className="managerCard">
      ...
      {loaded&&<div style={{marginTop:30,display:'grid',gap:24}}>
        ...
        <section><h2>Add Vacation or Sick Time</h2><form onSubmit={addPaidTimeOff} className="paidTimeOffForm"><select name="employeeId" required defaultValue=""><option value="" disabled>Select employee</option>{employees.map(e=><option key={e.id} value={e.id}>{e.name} (#{e.employeeNumber})</option>)}</select><select name="entryType" required defaultValue="vacation"><option value="vacation">Vacation</option><option value="sick">Sick</option></select><input name="entryDate" type="date" min={startDate} max={endDate} required/><input name="hours" type="number" min="0.25" max="24" step="0.25" defaultValue="8" required/><input name="note" placeholder="Note (optional)" maxLength={250}/><button className="primary" disabled={loading}>{loading?'Saving…':'Add to Spreadsheet'}</button></form></section>
        <section><h2>Paid Time Off</h2><div className="tableWrap"><table><thead><tr><th>Employee</th><th>Branch</th><th>Type</th><th>Date</th><th>Hours</th><th>Note</th></tr></thead><tbody>{filteredPaidTimeOff.map(r=><tr key={r.id}><td><strong>{r.employeeName}</strong><br/>#{r.employeeNumber}</td><td>{r.location}</td><td>{r.entryType==='vacation'?'Vacation':'Sick'}</td><td>{r.entryDate}</td><td>{r.hours.toFixed(2)}</td><td>{r.note||'—'}</td></tr>)}{filteredPaidTimeOff.length===0&&<tr><td colSpan={6}>No paid time off in this date range.</td></tr>}</tbody></table></div></section>
        <section><div className="sectionHeading"><h2>Weekly Summary</h2><div className="timecardTools"><label className="employeeSearch"><span>Search employee</span><input type="search" placeholder="Name or employee number" value={employeeSearch} onChange={e=>setEmployeeSearch(e.target.value)} /></label><label className="summarySort">Sort by<select value={summarySort} onChange={e=>setSummarySort(e.target.value as SummarySort)}><option value="employee-asc">Employee A–Z</option><option value="employee-desc">Employee Z–A</option><option value="location-asc">Branch</option><option value="hours-desc">Hours: highest first</option><option value="hours-asc">Hours: lowest first</option><option value="incomplete">Incomplete punches first</option></select></label></div></div><div className="tableWrap"><table><thead><tr><th>Employee</th><th>Location</th><th>Paid Hours</th><th>Unpaid Breaks</th><th>Status</th></tr></thead><tbody>{filteredSummaries.map(r=><tr key={r.employeeId}><td><strong>{r.employeeName}</strong><br/>#{r.employeeNumber}</td><td>{r.location}</td><td>{r.totalHours.toFixed(2)}</td><td>{r.breakHours.toFixed(2)}</td><td>{r.incomplete?'Incomplete punch':'Complete'}</td></tr>)}{filteredSummaries.length===0&&<tr><td colSpan={5}>{searchTerm?'No employee matches your search.':'No punches found for this pay period.'}</td></tr>}</tbody></table></div></section>
