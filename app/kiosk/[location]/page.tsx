...
type EmployeeState = { employeeId: string; firstName: string; status: 'clocked_in' | 'clocked_out'; breakStartedAt: string | null };
...
export default function BranchKioskPage() {
  ...
  return <main className="shell"><section className="card kioskCard">
    ...
    </> : <div className="employeePanel"><p className="welcome">Welcome, <strong>{employee.firstName}</strong></p><div className="status">Status: <strong>{employee.breakStartedAt ? 'On Break' : employee.status === 'clocked_in' ? 'Clocked In' : 'Clocked Out'}</strong></div>{employee.breakStartedAt ? <><div className="breakTimer"><span>Break time</span><strong>{breakTime(employee.breakStartedAt)}</strong></div><button className="breakEnd" disabled={busy} onClick={() => send('end_break')}>{busy ? 'Saving…' : 'End Break'}</button></> : <><button className={employee.status === 'clocked_in' ? 'danger' : 'primary'} disabled={busy} onClick={() => send(employee.status === 'clocked_in' ? 'clock_out' : 'clock_in')}>{busy ? 'Saving…' : employee.status === 'clocked_in' ? 'Clock Out' : 'Clock In'}</button>{employee.status === 'clocked_in' && <button className="breakStart" disabled={busy} onClick={() => send('start_break')}>Start Break</button>}</>}<button className="cancel" onClick={reset}>Cancel</button></div>}
